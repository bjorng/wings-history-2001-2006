...
%%
%%     $Id: e3d__tga.erl,v 1.1 2001/10/18 16:06:18 bjorng Exp $
%%
...
load_uncomp(<<Width:16/little,Height:16/little,BitsPP:8,Def:8,Image/binary>>) ->
    BytesPerPixel = BitsPP div 8,
    SpecOK = (Width > 0) and (Height > 0) and ((BitsPP == 32) or (BitsPP == 24)),
    if 
	SpecOK == false ->
	...
	true ->
	    ...
	    #e3d_image{width = Width, height = Height, type = Type,
		       bytes_pp = BytesPerPixel, alignment = 1,
		       image = RealImage}
    end.

load_comp(<<Width:16/little,Height:16/little,BitsPP:8,Def:8,CImage/binary>>) ->
    BytesPerPixel = BitsPP div 8,
    SpecOK = (Width > 0) and (Height > 0) and ((BitsPP == 32) or (BitsPP == 24)),
    if 
	SpecOK == false ->
...
save(Image0, FileName, Opts) ->
	...
	if 
		...
		{e3d_image:convert(Image0, b8g8r8, 1), 24, 0};
	    Image0#e3d_image.bytes_pp == 4 ->
		{e3d_image:convert(Image0, b8g8r8a8, 1), 32, <<8:4, 0:4>>}
	end,
    Head0 = <<0,0,2,0,0,0,0,0,0,0,0,0>> ,    
    Head1 = <<(Image#e3d_image.width):16/little, (Image#e3d_image.height):16/little,BitsPP:8,Def:8>> ,
