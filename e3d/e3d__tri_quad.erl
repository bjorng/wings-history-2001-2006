...
chopear(J, F, N, I, Acc) when J > N -> reverse(Acc);
...
tryfindear(F, N, I, Incr, Start, Angk, Vtab, Mode) when I > N -> none;
...
earloop(F, J, N, Angk, Vm1, V0, V1, Vtab) when J > N -> true;
...
joinislands(Face, [], Vtab) -> Face;
joinislands(#e3d_face{vs=Vs}=Face, Holes, Vtab) ->
	Hole = leftmostface(Holes, Vtab),
	Hrest = delete(Hole,Holes),
	Face1 = joinisland(Face, Hole, Vtab),
	joinislands(Face1, Hrest, Vtab).

joinisland(#e3d_face{vs=Vs}=Face, #e3d_face{vs=[Hv|_]=Hvs}=Hole, Vtab) ->
	F = list_to_tuple(Vs),
	N = size(F),
...
isdiagloop(J, F, N, V, Hv, Vtab) when J > N -> true;
...
ergraphe(E={A, B}, Tl, Tabs={Bord, TD, DD, Vtab}, Acc) ->
	case gb_sets:is_member(E, Bord) of
	true -> Acc;
	_ ->
		Er = {B, A},
...
segsintersect(IA, IB, IC, ID, Vtab) ->
	...
	true ->
		...
		{false, false} ->
			Z = sub2(B,C),
			{Ux,Uy}=U, {Vx,Vy}=V, {Wx,Wy}=W, {Zx,Zy}=Z,
			{T0,T1} = case Vx of
				0.0 -> {Wx/Vx, Zx/Vx};
				_ -> {Wy/Vy, Zy,Vy}
				end,
			(T0 > 0.0) and (T0 < 1.0) and (T1 > 0.0) and (T1 < 1.0);
